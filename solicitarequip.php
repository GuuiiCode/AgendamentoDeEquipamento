<!DOCTYPE html>
<?php
  include_once('scripts/conexao.php');
  include_once('scripts/verificaLogin.php');
  include_once('scripts/telaSolicitaEquipamentoP.php');
  include_once('scripts/menu.php');
  include_once('scripts/deslogar.php');
?>

<!DOCTYPE html>
<html lang="pt-br">
  <head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">

    <title>Solicitar Equipamento</title>

    <!-- Bootstrap core CSS -->
    <link href="vendor/bootstrap/css/bootstrap.min.css" rel="stylesheet">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/materialize/0.98.0/css/materialize.min.css">
    <link rel="stylesheet" href="style/style.css">
  </head>
  <body>
    <div class="container">
      <div class="col-lg-12 text-center">
        <h3 style="padding:40px 0">Solicitar Equipamento</h3>
        <table class="table table-hover responsive-table centered">
          <thead class="thead-dark">
            <tr>
              <th>Equipamento</th>
              <th>Quantidade</th>
              <th>Solicitar</th>
            </tr>
          </thead>
          <tbody>
            <?php 
              while ($reg = mysqli_fetch_array($rs)) {
                $descricao = $reg{1};
                $quantidade = $reg{2};
                $codigo = $reg{3};
                
                echo(
                  "<tr>
                    <td>$descricao</td>
                    <td>$quantidade</td>"
                );
                
                if ($quantidade <= 0 ) {
                  echo("<td>Sem Estoque</td>");
                } else {
                  echo(
                    "<td>
                      <form method='post' action='pedido.php'>
                        <input type='hidden' name='codigos' value='$codigo'>
                        <input type='hidden' name='quantidade' value='$quantidade'>
                        <button type='submit' name='solicitar' style='background-color:transparent;border:0px;'>
                          <img src='images/aprov.png'>
                        </button>
                      </form>
                    </td>
                  </tr>");
                }
              }
            ?>
          </tbody>
        </table>
      </div>
    </div>

    <!-- Bootstrap core JavaScript -->
    <script src="vendor/jquery/jquery.min.js"></script>
    <script src="vendor/bootstrap/js/bootstrap.bundle.min.js"></script>

  </body>
</html>
